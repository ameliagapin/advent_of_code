...
func (m move) do(stacks map[int][]string) map[int][]string {
	// stacks are 0-indexed in the map, the input is 1-indexed
	from := m.from - 1
	to := m.to - 1
	...
	index := len(stackFrom) - m.num
	items := stackFrom[index:]
	stackFrom = stackFrom[:index]
	stackTo = append(stackTo, items...)
